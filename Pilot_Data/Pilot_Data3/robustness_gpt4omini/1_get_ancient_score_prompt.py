    ...
    for k in data:
        ...
        for _, i in data[k].iterrows():
            ...
            for ss in sorted(list(sss.keys())):
                ...
                q += re.sub(r"\n+", "\n", i["模型生成+张改"]) + '\n\n'
                q += f'根据描述，请判断{i["行为"]}作为{k[:2]}，在“{ss}”角度的程度分数。“{ss}”角度指的是{sss[ss]["维度定义"]}分数满分为10分，0分是完全没有，10分是程度极高，需要在最后明确给出分数。\n'
                q += f'举例来说：分数低的表示{ss}{sss[ss]["负端例子"]}；分数高的表示{ss}{sss[ss]["正端例子"]}。\n'
                q += '请您考虑该行为的各个方面，包括这种行为的实施者、发生的场景，您所了解的这种行为的发生过程与危害，以及其它所有的此行为该有的特征。需要强调的是，您需要判断的是您认为大多数人是怎样看待这种行为的，而不是您自己对这种行为的看法。'
...
if __name__ == '__main__':
    ...
    open('data_ancient_scores_gpt4omini20240718_40dim_1234_1011_prompt.json', 'w').write(json.dumps(data, ensure_ascii=False, indent=2))
